...
def plot_2d_comp_multinom(model, data, vmin=None, vmax=None,
                          ...
                          adjust=True,residualfilenamepre="residuals.txt"):
    """
    ...
    plot_2d_comp_Poisson(model, data, vmin=vmin, vmax=vmax,
                         ...
                         adjust=adjust,residualfilenamepre=residualfilenamepre)
...
def plot_2d_comp_Poisson(model, data, vmin=None, vmax=None,
    ...
    if residual == 'Anscombe':
        resid = Inference.Anscombe_Poisson_residual(masked_model, masked_data,
                                              mask=vmin)
        pickle.dump(resid.data,open(residualfilenamepre+"array.pickle","wb"))
        f=open(residualfilenamepre+"array.txt",'a')
        for rowORcol in resid.data:
            for e in rowORcol:
                if e is None:
                    e="--"
                print >>f,str(e)+"\t",
...
#     b=flatresid.tolist()
    if residual == 'Anscombe':
        pickle.dump(flatresid.data,open(residualfilenamepre+"hist.pickle",'wb'))
        f=open(residualfilenamepre+"hist.txt",'a')
        for e in flatresid:
            print >>f,str(e)+" ",
        print >>f
        f.close()
